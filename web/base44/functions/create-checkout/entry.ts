...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Fixed-entitlement plan: quantity is always 1, ignore any client-sent quantity.
    const quantity = 1;
    const productName = "Masterpiece Premium";
    const price = "9.99"; // $9.99/month, resolved server-side — never trust a client price
    const currency = "USD";
    // Monthly auto-renewing subscription.
    const subscriptionInfo = {
      subscriptionSettings: { frequency: "MONTH" },
      title: "Masterpiece Premium",
      description: "Unlimited downloads and zero ads. Billed monthly.",
    };
