...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant: premium subscription → set the buyer's plan to "premium".
  // Idempotent: an update keyed on the user id, safe across duplicate deliveries.
  if (purchase.productId === "premium") {
    ...
    if (targetUserId) {
      await db.entities.User.update(targetUserId, { plan: "premium" });
      console.log("payments-webhook: granted premium", { targetUserId, checkoutId });
    } else {
      // Anonymous buyer with no app user yet — premium-status claims it by email once they sign up.
      console.warn("payments-webhook: no user to grant yet, will be claimed on signup", { checkoutId, buyerEmail });
    }
  }
  // ===== END APP-SPECIFIC =====
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke: mirror of the grant — set the buyer's plan back to "free". Idempotent.
  if (purchase.productId === "premium") {
    let targetUserId = purchase.appUserId ?? null;
    if (!targetUserId && purchase.buyerEmail) {
      const users = await db.entities.User.filter({ email: purchase.buyerEmail });
      targetUserId = users?.[0]?.id ?? null;
    }
    if (targetUserId) {
      await db.entities.User.update(targetUserId, { plan: "free" });
      console.log("payments-webhook: revoked premium", { targetUserId, subscriptionId });
    }
  }
  // ===== END APP-SPECIFIC =====
